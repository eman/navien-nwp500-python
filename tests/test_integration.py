...
class ProductionIntegrationTest:
    ...
    async def test_configuration(self) -> bool:
        ...
        try:
            ...
            if not self.config.email or not self.config.password:
                self.logger.error(
                    "❌ Email and password must be set via environment variables"
                )
    ...
    async def test_device_discovery(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"🏠 Found device: {device_name} (MAC: {self.device.mac_address})"
            )
    ...
    async def test_mqtt_connection(self) -> bool:
        ...
        try:
            ...
            reconnect_config = ReconnectConfig(
                max_retries=5, initial_delay=1.0, max_delay=10.0, jitter=True
            )
    ...
    async def test_data_retrieval(self) -> bool:
        ...
        try:
            ...
            # Configure reconnection for testing
            reconnect_config = ReconnectConfig(
                max_retries=3, initial_delay=1.0, max_delay=5.0, jitter=False
            )
            ...
            async def collect_data(status):
                ...
                self.logger.info(
                    f"   Tank: {charge}% | Temp: {temp}°F | Mode: {mode} | Power: {power}W"
                )
        ...
        try:
            ...
            try:
                ...
                await invalid_client.authenticate(
                    "invalid@email.com", "invalid_password"
                )
                ...
                self.logger.warning(
                    f"Unexpected error type for invalid auth: {type(e).__name__}"
                )
            finally:
            ...
            try:
                ...
                if devices:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Expected connection to invalid endpoint to fail"
                        )
                    except Exception:
                        self.logger.info(
                            "✅ Connection to invalid endpoint properly failed"
                        )
                ...
                self.logger.info(
                    f"✅ Error handling working as expected: {type(e).__name__}"
                )
    ...
    def print_results(self):
        ...
        if passed == total:
            ...
            self.logger.error(
                f"💥 {total - passed} tests failed. Review configuration and connectivity."
            )
