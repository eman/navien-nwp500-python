...
class TestHarMqttData:
    ...
    def test_mqtt_status_messages_structure(self):
        ...
        critical_fields = [
            'command', 'errorCode', 'operationMode', 
            'dhwTemperature', 'dhwChargePer', 'currentInstPower'
        ]
    ...
    def test_production_data_validation(self):
        ...
        assert operation_modes.issubset(valid_modes), f"Unexpected modes: {operation_modes - valid_modes}"
...
class TestDataSanitization:
    ...
    def test_authentication_tokens_sanitized(self):
        ...
        for field in token_fields:
            ...
            assert len(str(token_data[field])) > 10, f"Token field {field} appears empty"
    ...
    def test_user_info_sanitized(self):
        ...
        for response_key, response in data['api_responses'].items():
            if 'userId' in str(response):
                ...
                assert 'test@example.com' in response_str or 'test_user_' in response_str
