...
from navien_nwp500.models import (
    UserInfo,
    DeviceInfo, 
    DeviceStatus,
    DeviceFeatures
...
class TestAuthenticationParser:
    ...
    def test_sign_in_response_parsing(self, har_test_data, mock_config):
        ...
        for token_type in required_tokens:
            ...
            assert token_data[token_type] == f'REDACTED_TOKEN_{len(token_data[token_type])}'
...
class TestDeviceListParser:
    ...
    def test_device_list_parsing(self, har_test_data):
        ...
        if devices:  # If we have device data
            ...
            required_fields = ['homeSeq', 'macAddress', 'additionalValue', 'deviceType', 'deviceName']
...
class TestMqttStatusParser:
    ...
    def test_mqtt_status_structure(self, har_test_data):
        ...
        core_fields = [
            'command', 'errorCode', 'subErrorCode', 'operationMode',
            'dhwTemperature', 'dhwTemperatureSetting', 'dhwChargePer',
            'currentInstPower', 'wifiRssi'
        ]
        ...
        temp_fields = [
            'tankUpperTemperature', 'tankLowerTemperature',
            'dischargeTemperature', 'ambientTemperature'
        ]
    ...
    def test_production_data_insights(self, har_test_data):
        ...
        for i, status_msg in enumerate(mqtt_data[:5]):  # Test first 5 messages
            ...
            if dhw_temp > 0:
                assert 70 <= dhw_temp <= 140, f"DHW temperature out of range: {dhw_temp}°F"
    ...
    def test_tou_response_structure(self, har_test_data):
        ...
        if schedule:
            ...
            if intervals:
                ...
                interval_fields = ['priceMin', 'priceMax', 'startHour', 'endHour', 'decimalPoint']
...
class TestFirmwareInfoParser:
    ...
    def test_firmware_info_structure(self, har_test_data):
        ...
        if firmwares:
            ...
            required_fields = [
                'macAddress', 'additionalValue', 'deviceType',
                'curSwCode', 'curVersion'
            ]
            for field in required_fields:
...
class TestParserRobustness:
    ...
    def test_field_type_validation(self, har_test_data):
        ...
        # Test integer fields
        int_fields = [
            'command', 'errorCode', 'operationMode', 'dhwTemperature',
            'currentInstPower', 'dhwChargePer', 'wifiRssi'
        ]
    ...
    def test_status_monitoring_workflow(self, har_test_data):
        ...
        for status_msg in mqtt_data[:3]:  # Test first 3 messages
            ...
            for field in monitoring_fields:
                assert field in status_data, f"Critical monitoring field missing: {field}"
