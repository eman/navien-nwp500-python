...
class NaviLinkMQTT:
        ...
        while self._retry_count < self._reconnect_config.max_retries:
            try:
                ...
                if self._retry_count >= self._reconnect_config.max_retries:
                    logger.error(f"❌ Max connection retries ({self._reconnect_config.max_retries}) exceeded")
                    raise
                
                # Calculate backoff delay with jitter
                delay = min(
                    self._reconnect_config.initial_delay * (self._reconnect_config.backoff_multiplier ** (self._retry_count - 1)),
                ...
                logger.warning(f"⏳ Connection attempt {self._retry_count} failed: {e}. Retrying in {delay:.1f}s...")
                await asyncio.sleep(delay)
    
    async def _connection_monitor(self):
        """Monitor connection health and auto-reconnect if needed."""
        while self._connection_state not in [ConnectionState.DISCONNECTED, ConnectionState.FAILED]:
            try:
    ...
    async def _subscribe_with_retry(self, topic: str, qos: int = 1, max_retries: int = 3) -> bool:
    ...
    def _handle_device_response_sync(self, topic: str, message_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"🔍 MQTT Response Data: {json.dumps(message_data, indent=2)[:500]}...")
            logger.debug(f"📨 Response data keys: {list(response_data.keys()) if response_data else 'No response data'}")
            ...
            if session_id and session_id in self._pending_responses:
                ...
                logger.debug(f"📨 Unmatched response session: {session_id} (pending: {list(self._pending_responses.keys())})")
    ...
    def _extract_and_log_status_data(self, response_data: Dict[str, Any]):
        ...
        try:
            ...
            interesting_fields = [
                'dhwTemperature', 'dhwTemperatureSetting', 'dhwCharge', 
                'operationMode', 'heatPumpStatus', 'elementStatus',
                'outsideTemperature', 'errorCode', 'subErrorCode',
    ...
    def _parse_channel_status(self, channel_data: Dict[str, Any]) -> Optional[DeviceStatus]:
        ...
        try:
            ...
            return DeviceStatus(
                ...
                freeze_protection_use=1 if channel_data.get("freezeProtection", False) else 0,
                ...
                **{field: 0 for field in DeviceStatus.__dataclass_fields__ 
                   if field not in ['command', 'outside_temperature', 'special_function_status',
                                   ...
                                   'dhw_charge_per', 'program_reservation_use', 'smart_diagnostic', 
                                   'fault_status1', 'fault_status2', 'wifi_rssi', 'eco_use', 
                                   'dhw_target_temperature_setting', 'heat_pump_status', 
                                   'resistance_heater_status', 'defrost_mode']}
            )
        except Exception as e:
    ...
    def _parse_device_status(self, data: Dict[str, Any]) -> Optional[DeviceStatus]:
        ...
        try:
            ...
            status_data = data.get("status", data)  # Handle both nested and direct status
            ...
            device_status = DeviceStatus(
                ...
                dhw_charge_per=status_data.get("dhwChargePer", 0),  # Tank charge percentage
                ...
                current_heat_use=status_data.get("currentHeatUse", 0),  # Resistance heater
                ...
                heat_upper_use=status_data.get("heatUpperUse", 0),  # Upper heating element
                heat_lower_use=status_data.get("heatLowerUse", 0),  # Lower heating element
                ...
                heat_pump_status=status_data.get("compUse", 0),  # Alias for heat pump status
                resistance_heater_status=max(status_data.get("heatUpperUse", 0), status_data.get("heatLowerUse", 0)),  # Either element on
    ...
    async def send_device_command(
        ...
        if self._connection_state != ConnectionState.CONNECTED:
            raise MQTTError(f"Not connected to MQTT (state: {self._connection_state.value})")
    ...
    async def _publish_with_retry(self, topic: str, payload: bytes, qos: int = 1, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"⚠️ Publish attempt {attempt + 1} failed, retrying...")
                    await asyncio.sleep(2 ** attempt)  # Exponential backoff
                    
            except Exception as e:
                if attempt == max_retries - 1:
                    raise
                logger.warning(f"⚠️ Publish attempt {attempt + 1} error: {e}, retrying...")
                await asyncio.sleep(2 ** attempt)
        ...
        while self._monitoring and self._connection_state == ConnectionState.CONNECTED:
            try:
                ...
                if error_count >= max_errors:
                    logger.error(f"❌ Max polling errors ({max_errors}) reached, stopping monitoring")
                    break
    ...
    async def _poll_device_status(self):
        ...
        try:
            ...
            command_data = {
                "clientID": self._session_id,
                "sessionID": request_session_id, 
                "deviceID": self._device.controller_id or self._device.mac_address,  # Use controller_id or MAC as fallback
            ...
            success = await self._aws_connection.publish(topic, payload, qos=0)  # QoS 0 for fire-and-forget
    ...
    def is_connected(self) -> bool:
        ...
        return (
            ...
            self._aws_connection and 
            self._aws_connection.is_connected
        )
