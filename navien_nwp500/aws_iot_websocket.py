...
try:
    ...
    raise ImportError(
        "AWS IoT SDK not available. Install with: pip install awsiotsdk>=1.21.0"
    )
...
class AWSIoTWebSocketConnection:
    ...
    def __init__(
        ...
        logger.info(
            f"Using {'MQTT5 (experimental)' if self.use_mqtt5 else 'MQTT3 (stable)'} client"
        )
    ...
    async def _connect_mqtt5(self, credentials_provider):
        ...
        try:
            ...
            def on_connection_failure(connack_packet):
                ...
                self.state.connected = False
                if not self.state.connect_future.done():
                    self.state.connect_future.set_exception(
                        Exception(f"MQTT5 connection failed: {reason}")
                    )
            ...
            def on_message_received(publish_packet):
                ...
                try:
                    ...
                    logger.debug(
                        f"📨 Received MQTT5 message on {topic}: {len(payload)} bytes"
                    )
    ...
    async def _connect_mqtt3(self, credentials_provider):
        ...
        def on_connect(connection, return_code, session_present, **kwargs):
            if return_code == mqtt.ConnectReturnCode.ACCEPTED:
                ...
                if not self.state.connect_future.done():
                    self.state.connect_future.set_exception(
                        Exception(f"Connect failed: {return_code}")
                    )
    ...
    async def subscribe(
        self, topic: str, qos: int = 1, callback: Optional[Callable] = None
    ) -> bool:
        """
    ...
    async def _subscribe_mqtt5(
        ...
        def on_subfail(suback_packet):
            ...
            if not subscribe_future.done():
                subscribe_future.set_exception(
                    Exception(f"Subscription failed: {reason}")
                )
        ...
        try:
            ...
            subscribe_packet = SubscribePacket(topic_filters=[(topic, mqtt5_qos)])

            # Subscribe with MQTT5
            subscribe_task = self.client.subscribe(
                subscribe_packet=subscribe_packet, on_suback=on_suback
            )
    ...
    async def _subscribe_mqtt3(
        ...
        sub_future, packet_id = self.connection.subscribe(
            topic=topic, qos=qos_level, callback=on_message
        )
    ...
    async def publish(self, topic: str, payload: bytes, qos: int = 1) -> bool:
        ...
        logger.debug(
            f"Publishing to topic: {topic} (payload size: {len(payload)} bytes)"
        )
    ...
    async def _publish_mqtt5(self, topic: str, payload: bytes, qos: int = 1) -> bool:
        ...
        if qos == 0:
            try:
                publish_packet = PublishPacket(
                    topic=topic, payload=payload, qos=QoS.AT_MOST_ONCE
                )
        ...
        try:
            ...
            publish_packet = PublishPacket(topic=topic, payload=payload, qos=mqtt5_qos)

            # Publish with MQTT5
            publish_task = self.client.publish(
                publish_packet=publish_packet, on_puback=on_puback
            )
        ...
        if qos == 0:
            try:
                ...
                pub_future, packet_id = self.connection.publish(
                    topic=topic, payload=payload, qos=qos_level
                )
                return True
            except Exception as e:
                logger.error(f"Failed to publish to {topic}: {e}")
    ...
    async def send_control_command(
        ...
        # Control topic
        control_topic = (
            f"cmd/{self.device.device_type}/navilink-{self.device.mac_address}/ctrl"
        )
    ...
    def is_connected(self) -> bool:
        ...
        return self.state.connected and (
            self.connection is not None or self.client is not None
        )
