...
class NaviLinkClient:
        ...
        try:
            async with self._session.post(
                ...
                json={
                    ...
                    "count": 20,
                    "userId": (
                        self._auth.user_info.email if self._auth.user_info else ""
                    ),
                },
                ...
                if not devices_data:
                    logger.warning(
                        f"No devices found in response. Full response keys: {list(data.keys())}"
                    )

                self._devices = [
                    NaviLinkDevice(
                        client=self, device_data=device_data, session=self._session
                    )
    ...
    async def get_tou_info(
        self, additional_value: str, controller_id: str, mac_address: str
    ) -> TOUInfo:
        """
